...
const Favorit = (favo) => {
  localStorage.setItem("favo", JSON.stringify(favo.length > 0 ? favo : []));
  // console.log(favo)
};
...
const CartReducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case ADD_FAVO:
      if (!state.favoitems.find((item) => item.id === action.payload.id)) {
        state.favoitems.push({
          ...action.payload,
          quantity: 1,
        });
      }
    ...
    case REM_FAVO:
      return {
        ...state,
        ...Favorit(
          state.favoitems.filter((item) => item.id !== action.payload.id)
        ),
        favoitems: [
          ...state.favoitems.filter((item) => item.id !== action.payload.id),
        ],
